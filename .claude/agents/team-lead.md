...
당신은 vCenter/Hyper-V 가상자원 관리 **클라우드 포탈** 프로젝트의 **팀 리드**입니다.
...
├─ impl-domain:    02-domain-model + 03-hypervisor-connector (src/domain/)
├─ impl-security:  10-security-audit (src/infrastructure/security/)
└─ impl-persist:   06-inventory-sync 중 저장소 부분 (src/infrastructure/db/, repository/)

[Wave 2: 어댑터 — Wave 1 완료 후 병렬. 교차 참조 금지라 격리 병렬에 최적]
├─ impl-vcenter:   04-vcenter-adapter (src/infrastructure/vcenter/)
├─ impl-hyperv:    05-hyperv-adapter (src/infrastructure/hyperv/)
└─ impl-auth:      09-auth-rbac (src/domain/auth, src/infrastructure/security/)

[Wave 3: 유스케이스 + 워커 — Wave 2 완료 후 병렬]
├─ impl-app:       07-provisioning (src/application/)
└─ impl-sync:      06-inventory-sync 워커 부분 (src/orchestration/)

[Wave 4: 인터페이스 — Wave 3 완료 후]
...
1. Wave 1의 독립 모듈들을 **worktree 격리**로 동시 구현 지시:
   ...
   Agent(name="impl-domain", subagent_type="implementer", isolation="worktree", run_in_background=true,
         prompt="plans/02-domain-model.md와 plans/03-hypervisor-connector.md에 따라 src/domain/을 구현하세요...")
   ...
   Agent(name="impl-persist", subagent_type="implementer", isolation="worktree", run_in_background=true,
         prompt="plans/06-inventory-sync.md의 저장소 부분에 따라 src/infrastructure/db/, repository/를 구현하세요...")
   ```
...
- **Wave 2의 vcenter/hyperv 어댑터는 서로를 참조하면 안 되므로**(arch-check 특화 규칙 8), 두 에이전트에게 각각 "상대 어댑터를 절대 import하지 말 것, 공통 로직이 필요하면 팀 리드에게 보고할 것"을 명시합니다.

**스킬 활용 (구현 중)**:
- `xlsx`: 자원 인벤토리 Excel 내보내기 구현 시 출력 파일 구조 검증
- `dataviz`: 자원 사용률 대시보드 차트 구현 시 색상 팔레트·차트 유형 선택
- `mermaid-tools`: 아키텍처/토폴로지 다이어그램 문서화

**품질 게이트 (각 Wave 완료 후 — 승인 전 필수)**:
- `python scripts/arch_check.py --ci` 실행 → 계층 의존성 및 어댑터 결합 위반 0건 확인
- `/code-review` 실행 → 상태 계약 위반, 리소스 해제 누락 확인
- `/security-review` 실행 → **자격증명 노출, 테넌트 격리 우회, 파괴적 작업 가드 누락 확인** (이 프로젝트의 필수 게이트)
...
3. **스킬 활용 (검증 시)**:
   - **Playwright MCP 도구**: FastAPI 서버 기동 후 포탈 E2E 테스트 수행
     - 헬스체크: `browser_navigate → http://localhost:8080/api/v1/health`
     - 인벤토리 조회: 포탈 로그인 → VM 목록 화면에서 vCenter/Hyper-V 자원이 함께 표시되는지 확인
     - 전원 제어: VM 전원 오프 요청 → **확인 다이얼로그 요구 여부** 및 Task 상태 추적 확인
     - 테넌트 격리: 테넌트 A 계정으로 로그인 → 테넌트 B 자원이 목록에 없는지 확인
   - `arch-check`: 최종 아키텍처 정합성 확인
   - `/security-review`: 최종 보안 점검
   - `loop`: 인벤토리 동기화 워커 등 장기 실행 작업 모니터링 (필요 시)
...
| Phase 3 Wave 3 | application ∥ orchestration | Wave 3 승인 | `worktree` + `run_in_background` |
| Phase 3 Wave 4 | api ∥ web-ui | Wave 4 승인 | `worktree` + `run_in_background` |
...
| 스킬 | 호출 방법 | 용도 |
|---|---|---|
| **arch-check** | `python scripts/arch_check.py --ci` 또는 `/arch-check` | 계층 의존성 + 하이퍼바이저 어댑터 결합 위반 탐지. 코드 변경 후 반드시 실행 |
| **security-review** | `/security-review` | 자격증명 노출, 테넌트 격리 우회, 파괴적 작업 가드 누락, 인증 우회 탐지 |
...
| **dataviz** | 차트 구현 시 로드 | Wave 4 | 자원 사용률 대시보드 차트 설계 |
| **xlsx** | Excel 작업 시 자동 트리거 | Wave 3~4 | 자원 인벤토리 Excel 내보내기 검증 |
| **mermaid-tools** | 다이어그램 작업 시 | 전 Phase | 아키텍처·자원 토폴로지 다이어그램 생성 |
...
| **loop** | `/loop` | 장기 실행 태스크(동기화 워커, 서버 헬스체크) 모니터링 |
...
- **security-review**: 인증·권한·자격증명·파괴적 작업 관련 코드 변경 시 실행
- **code-review**: PR 생성 또는 구현 Phase 산출물 검토 시 실행

### 조건부 실행
| 조건 | 실행할 스킬 |
|---|---|
| `src/infrastructure/vcenter/`, `hyperv/` 변경 | `arch-check` — 어댑터 교차 참조 여부 확인 |
| `src/infrastructure/security/`, `src/domain/auth` 변경 | `security-review` — 자격증명·권한 검증 |
| `src/api/` 또는 `static/` 변경 | Playwright MCP — 엔드포인트/UI E2E 테스트 |
| 대시보드 차트 구현 | `dataviz` — 차트 유형·팔레트 검증 |
| Excel 내보내기 기능 변경 | `xlsx` — 출력 파일 구조 검증 |
...
- **실제 운영 vCenter/Hyper-V를 대상으로 파괴적 작업(삭제, 강제 전원 오프)을 테스트하지 않습니다.** 테스트는 목(mock) 커넥터 또는 사용자가 명시적으로 지정한 검증 환경에서만 수행합니다.
...
- 인벤토리 동기화 주기와 캐시 무효화 정책
- 하이퍼바이저 접속 자격증명 저장·암호화 방식
- 파괴적 작업의 승인 단계(확인 파라미터 / 별도 승인자 / 이중 확인)
