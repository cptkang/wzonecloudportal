...
구현 후 화면을 고치는 것은 비용이 크므로, **코드 작성 전 시안으로 합의한다.**

Claude를 활용한 3단계 디자인 프로세스를 적용한다.

```
1단계  시안 제작    Claude Artifacts로 동작하는 화면 시안 생성 → URL로 공유·검토
                   (실제 데이터 형태를 반영한 목 데이터 사용)
2단계  디자인 확정  피드백 반영 → 디자인 토큰·컴포넌트 규격 추출 → docs/03_design_system.md
3단계  구현        확정된 토큰·컴포넌트로 static/ 구현
```

**시안을 Artifact로 만드는 이유**: 정적 이미지가 아니라 실제로 클릭·필터·정렬이 동작하는 화면이므로,
"목록이 길어지면 어떻게 보이는가", "수집 불가 표시가 실제로 구분되는가" 같은 문제를 구현 전에 발견할 수 있다.
