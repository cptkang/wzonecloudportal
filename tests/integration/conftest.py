"""통합 테스트 픽스처 — **실제 PostgreSQL**을 쓴다.

JSONB·INET·부분 인덱스·pg_trgm이 스키마에 들어 있어 SQLite로는 검증되지 않는다 (D-013).

스키마는 **Alembic으로 만든다** — 매 실행마다 마이그레이션 자체가 검증되고
모델과 마이그레이션의 드리프트가 드러난다.

이 파일의 `autouse` 픽스처는 `tests/integration/` 아래에만 적용된다.
루트 `conftest.py`에 두면 단위 테스트까지 DB를 요구하게 된다 (T1).
"""
