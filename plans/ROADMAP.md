...
| # | 확인할 것 | 실패 시 영향 |
|---|---|---|
| 1 | **대상 vCenter 실제 버전 분포 — 하한·상한 양방향** | 6.5 미만이 있으면 CST-10 재검토. **9.x(VCF 9)가 있으면 §15.5 전체가 필수**. 속성 경로 전면 재확인 |
| 2 | **pyVmomi 버전 호환** (조사 §11-9) | 최신 pyVmomi(9.1.0.0)는 **직전 4개 릴리스만 지원** — 6.5 보증 없음. 붙지 않으면 버전 하향 고정 필요 (계획 04 §14.2) |
...
### 15.5 상한(VCF 9.x) 대응 — 2026-08-19 추가

계획 04는 지원 **하한(6.5)** 기준으로만 작성되어 있었고 상한 검증 항목이 없었다.
근거와 상세는 **계획 04 §14**에 있다. **§15.1-1에서 9.x가 없다고 확인되면 아래는 보류한다.**

| # | 확인할 것 | 실패 시 영향 |
|---|---|---|
| 19 | **수집 계정이 로컬 SSO(`vsphere.local`) 계정인가** (계획 04 §3.2) | vCenter 9.0은 **사용자명+암호 단독 로그인을 차단**한다. 페더레이션 계정이면 **연결 등록 자체가 불가** → 토큰 인증(`SmartConnect(token=...)`) 착수 판단 |
| 20 | TLS 프로파일 (`COMPATIBLE` / `NIST_2024_TLS_13_ONLY`) | 후자면 클라이언트 TLS 1.3 필수. VMCA 자체 서명 여부는 §15.1-5와 함께 본다 |
| 21 | `vim.Network` 뷰의 **MoRef 접두사 분포** (계획 04 §5.3) | DVPG·OpaqueNetwork가 함께 반환되므로 **중복 레코드** 발생. 제거 규칙 확정 |
| 22 | NSX 세그먼트의 `config.backingType` 실값 (계획 04 §5.3) | VCF는 NSX 기본 탑재. 값이 없으면 일반 포트그룹과 구별 불가 |
| 23 | vSAN 데이터스토어의 `summary.uncommitted` 유무 (계획 04 §5.1) | 없으면 오버커밋 계산 불가 → `provisioned`를 `None`으로 유지 |
| 24 | vCLS·Supervisor/VKS·템플릿 VM 비율 (계획 04 §5.4) | 목록 기본 필터·리포트 집계 왜곡 정도 판단 |
| 25 | `config.product.name` 실값 (**ESX** / ESXi) | VCF 9.0에서 제품명 환원. 조회·리포트의 `"ESXi"` 문자열 매칭 존재 여부 점검 |

> **vcsim으로는 이 절을 검증할 수 없다.** vcsim은 vSphere 6.5로 응답한다 (계획 04 §13.3).

## 16. 산출물

1. **`docs/04_field_validation.md` 신규 작성** — 위 25개 항목의 실측 결과와 측정 조건(일시·대상·건수)
2. `docs/00_research_notes.md` §11 갱신 — 해소된 항목 표시
3. `spec.md` 갱신 — NFR-104(규모)·NFR-105(수집 주기)의 `[TODO]` 해소, **CST-10에 지원 하한과 상한을 함께 확정** (현재 하한만 정의됨 — 계획 04 §14.4, D-020)
...
| **참조** | **계획 15 (이 단계의 실행 계획 — 표시 정보 카탈로그·수정 범위 전체)**, 계획 04 §6.3, 06 §2.4·2.9, 07 §4, 11 §15·16 |
